			...
			if(n < 0){
				...
				if(strcmp(server_message, "OK\n") == 0){
					...
					//printf("Did we get here?\n");
						sprintf(read_buffer, "%s%s", read_buffer, server_message);
					}
					else read_buffer = server_message;
					char* tok = read_buffer;
					char delimit[] = " \n";
					//printf("Before tok\n");
					//printf("Read_buffer before tok:%s\n", read_buffer);
					//tok = strtok(read_buffer, "\n");
					//printf("tok after tok:%s\n", tok);
					//tok = strtok(read_buffer, "\n");
					//read_buffer = strtok(NULL, "");
					//printf("Read_buffer after tok:%s\n", read_buffer);
					while((tok = strtok(read_buffer, "\n")) != NULL){
						read_buffer = strtok(NULL, "\n");
						//printf("Tok after tok:%s\n", tok);
						char* command = strtok(tok, " ");
						//printf("Read_buffer after command:%s\n", read_buffer);
						//printf("Tok after tok:%s\n", tok);


						char* name = strtok(NULL, " ");
						//printf("Read_buffer after name:%s\n", read_buffer);
						//printf("Tok after tok:%s\n", tok);

						char* message = strtok(NULL, "\n");
						//printf("Read_buffer after message:%s\n", read_buffer);
						//printf("Tok after tok:%s\n", tok);
...
//						printf("Read_buffer after tok2%s\n", read_buffer);
//						tok = strtok(read_buffer, "\n");
//						//while((tok = strtok(read_buffer, "\n")) != NULL){
//							printf("Tok after tok2:%s\n", tok);
//							command = strtok(tok, " ");
//							name = strtok(NULL, " ");
//							message = strtok(NULL, "\n");
...
//							tok = NULL;
					...
					read_buffer = strtok(NULL, "");
//					while((tok = strtok(NULL, delimit)) != NULL){
//						//TODO READ FULL BUFFER
//						//tok = strtok(NULL, "\n");
